...
public class LinkedVector implements Vector, Cloneable, Serializable{
    ...
    private Nod goToElement(int index) {
        Nod result = head;
        int i = 0;
        while (i != index) {
            result = result.next;
    ...
    private void insertElementBefore(Nod current, Nod newNod) {
        newNod.next = current;
    ...
    private void deleteElement(Nod current) {
        if (size == 1) {
            head = null;
	} else {
    ...
    @Override
    public void sum(Vector sVector) throws IncompatibleVectorSizesException {
        if (getSize() != sVector.getSize()) {
            throw new IncompatibleVectorSizesException();
        } else {
            Nod current = head;
            for (int i = 0; i < getSize(); i++) {
                current.element = sVector.getElement(i) + current.element;
                current = current.next;
